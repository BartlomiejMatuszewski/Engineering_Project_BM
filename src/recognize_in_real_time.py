...
from utilities import KeyPointClassifier as KPC
...
def run(model: str,
    ...
    # its a little bit stupid but to not break any good programming rules you have to create inside main save fun
    ...
    def save_landmarker_result(result: vision.GestureRecognizerResult,
                               unused_output_image: mp.Image,
                               timestamp_ms: int):
        global FPS
        global COUNTER
    ...
    classifier = KPC.KeyPointClassifier()

    # ------------------------------------------------------------------------------------------------------------------
    # FIXME: a little experiment
    landmarker_model = os.path.join("../models/hand_landmarker.task")

    base_landmarker_opts = python.BaseOptions(model_asset_path=landmarker_model)

    landmarker_opts = vision.HandLandmarkerOptions(
        base_options=base_landmarker_opts,
        running_mode=vision.RunningMode.LIVE_STREAM,
        num_hands=num_hands,
        min_hand_detection_confidence=min_hand_detection_confidence,
        min_hand_presence_confidence=min_hand_presence_confidence,
...
def main():
    # Here Shit happens with parameters, I don't even know why
